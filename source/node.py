...
class Node:
    ...
    def listen(self):
        while True:
            ...
            t = Thread(target=self.handle_requests, args=(msg, addr))
            t.start()
    ...
    def set_send_mode(self, filename: str):
        ...
        if self.is_in_send_mode:    # has been already in send(upload) mode
            ...
            log_content = f"You are waiting for other nodes' requests!"
            log(node_id=self.node_id, content=log_content)
    ...
    def split_file_owners(self, file_owners: list, filename: str):
        ...
        owners = sorted(owners, key=lambda x: x[1])

        to_be_used_owners = owners[:config.constants.MAX_SPLITTNES_RATE]
        # 1. ask the size of the file from peers
        ...
        log_content = f"The file {filename} has size of {file_size} bytes"
        log(node_id=self.node_id, content=log_content)

        # 2. split it equally among peers to download chunks of it from them
        ...
        log_content = "All the chunks of {} has downloaded from neighboring peers".format(filename)
        log(node_id=self.node_id, content=log_content)
    ...
    def set_download_mode(self, filename: str):
        ...
        if os.path.isfile(file_path):
            ...
            log_content = f"You just started to download {filename}. Search it in torrent!"
            log(node_id=self.node_id, content=log_content)
...
def run(args):
    ...
    # Create a thread to periodically informs the tracker to tell it is still in the torrent.
